...
def calc_pressure(dc, P, u_ishift_star, v_jshift_star):
    ...
    # The velocities need to be adjusted to account for the actual equation.
        # Currently, the ghost cell velocities are just being used. This is NOT
        # correct. The tangential wall velocity needs to be used.
    # I'm forcing this to be true with hard coding
    # print(u_ishift_star[:,0])
    # u_ishift_star[:,0:1] = 0
    # print(u_ishift_star[:,0:2])
    # u_ishift_star[:,-1] = 0
    # v_jshift_star[:,:] = 0

    # plt.figure()
    # plt.contourf(u_ishift_star)
    # plt.show()

    while not conv and count < 100 or count <= 10:
...
for i in range(len(P[:,2])):
    P[i,1:-1] =  dc.P - (dc.x_grid[0] - dc.x_grid[i])*-2.4
...
u_ishift = np.zeros(u.shape, dtype=numpy_dtype) + u
v_jshift = np.zeros(v.shape, dtype=numpy_dtype) + v
u_ishift_star = np.zeros(u.shape, dtype=numpy_dtype)
v_jshift_star = np.zeros(v.shape, dtype=numpy_dtype)
...
while t < dc.T: # and not user_done:
    ...
    v_jshift[:,0] = 0
    # plt.figure()
    # plt.contourf(x_vals,y_vals,v_jshift.T)
    # plt.show()
    A_x[1:-1,1:-1] = A_x_fun(dc.h,u_ishift,v_jshift)
    A_y[1:-1,1:-1] = A_y_fun(dc.h,u_ishift,v_jshift)
    D_x[1:-1,1:-1] = D_fun(dc.h, u_ishift)
    D_y[1:-1,1:-1] = D_fun(dc.h, v_jshift)

    # Predictor Step
    u_ishift_star = vel_star(u_ishift, dc.dt, A_x, nu, D_x) # u + dc.dt * ( -A_x + nu * D_x )
    v_jshift_star = vel_star(v_jshift, dc.dt, A_y, nu, D_y) #v + dc.dt * ( -A_y + nu * D_y )

    u_ishift_star = set_ghost(dc.domain_map, u_ishift_star, dc.u_B, source=dc.u_S)
    v_jshift_star = set_ghost(dc.domain_map, v_jshift_star, dc.v_B, source=dc.v_S)
    # u_ishift_star[0,:] = u_prof
    # u_ishift_star[-1,:] = u_prof
    v_jshift_star[:,-1] = 0
    v_jshift_star[:,-2] = 0
    v_jshift_star[:,0] = 0
    ...
    if pressure_solve == "gradient":
        ...
        P = calc_pressure(dc, P, u_ishift_star, v_jshift_star)

        dP_x[1:-1,1:-1] = (P[2:,1:-1] - P[1:-1,1:-1])/dc.h
        dP_y[1:-1,1:-1] = (P[1:-1,2:] - P[1:-1,1:-1])/dc.h
        dP_x = np.round(dP_x,6)
        dP_y = np.round(dP_y,6)
        # dP_y[:,:] = 0
        # dP_x = set_ghost(dc.domain_map,dP_x, dc.u_B, type="pressure",source=dc.dP_x_S)
        # dP_y = set_ghost(dc.domain_map,dP_y, dc.u_B, type="pressure",source=dc.dP_y_S)

        # plt.plot(dP_x.T[1:-1,1:-1],y_vals[1:-1])
        # plt.show()
        ...
        # plt.figure()
        # plt.contourf(x_vals[1:-1],y_vals[1:-1],P[1:-1,1:-1].T/np.max(P[1:-1,1:-1]))
        # plt.contourf(x_vals[1:-1],y_vals[1:-1],dP_y[1:-1,1:-1].T)
        # plt.show()
    ...
    v_jshift = vel_ishift_fun(v_jshift_star, dc.dt, rho, dc.h, dP_y, dc.F_y)

    # Applying boundaries to the new shifted velocities
    u_ishift = set_ghost(dc.domain_map,u_ishift,dc.u_B,source=dc.u_S)
    v_jshift = set_ghost(dc.domain_map,v_jshift,dc.v_B,source=dc.v_S)
    # u_ishift[0,:] = u_prof
    # u_ishift[-1,:] = u_prof
    v_jshift[:,-1] = 0
    v_jshift[:,-2] = 0
